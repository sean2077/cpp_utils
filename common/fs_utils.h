 ...
 */
    inline std::string path_join(const std::initializer_list<std::string> &paths)
    {
        ...
        for (const auto &path : paths) {
            if (!result.empty() && result.back() != path_separator && path.front() != path_separator) {
 ...
 * @return std::string The joined path.
 */
    template <typename... Args> inline std::string path_join(const std::string &path1, Args... paths)
    {
 ...
 */
    inline bool makedirs(const std::string &path, mode_t mode = S_IRWXU | S_IRWXG | S_IRWXO)
    {
        if (path.empty()) {
            return false;
        }
        std::string::size_type pos = 0;
        while ((pos = path.find_first_of(path_separator, pos + 1)) != std::string::npos) {
 ...
 */
    inline bool list_dir(const std::string &path, std::vector<std::string> &entries)
    {
        entries.clear();
#ifdef _WIN32
 ...
 *
 * This function takes a path string and returns a pair of strings: the directory
 * component and the file component. The directory component is the portion of the
 * path up to (but not including) the final path separator. The file component is
 * the portion of the path after the final path separator.
 *
 ...
 */
    inline std::string basename(const std::string &path)
    {
        ...
        // 如果路径只有一个字符，则返回该字符
        if (path.size() == 1) {
            return (path[0] == path_separator) ? std::string(1, path_separator) : std::string(1, path[0]);
        }
...
/**
 * @brief Recursively walks a directory tree and returns information about each directory and file encountered
 *
 * The function walks a directory tree, and for each directory and file it encounters, returns information about the
 * directory or file in a tuple. The returned vector contains a tuple for each directory visited, and each tuple
 ...
 *
 * The function accepts a callback function, which will be called for each directory visited. The callback function can
 * be used to customize the behavior of the directory traversal.
 *
 * @param path The path to the directory to start the traversal from
 * @return A vector containing a tuple for each directory visited. Each tuple contains three elements: the absolute
 * path of the directory, a vector of the names of the subdirectories in the directory, and a vector of the names of the
...
/**
 * @brief Recursively walk through a directory and execute the callback function for each file encountered.
 *
 * @param path The path to the directory to walk through.
 * @param cb The callback function to execute for each file encountered. The absolute path of the file is passed as the
 * argument.
 */
    inline void walkdir(const std::string &path, const std::function<void(const std::string &)> &cb)
    {
#ifdef _WIN32
 ...
 */
    ...
    HANDLE hFile =
        CreateFile(path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
 ...
 */
    ...
    {
        static const std::set<std::string> image_exts = { "bmp", "gif", "jpeg", "jpg", "png", "svg", "tif", "tiff", "webp" };
 ...
 */
    ...
    {
        static const std::set<std::string> video_exts = { "avi", "mp4", "mkv", "mov", "wmv", "flv", "f4v",  "rmvb", "rm",
...
///  判断是否为在线视频
    inline bool is_online_video(const std::string &url)
    {
        ...
        return std::any_of(remote_protocols.begin(), remote_protocols.end(),
...
/// 读取整个文件内容到string
    inline void read_file_to_string(const std::string &infile, std::string &outstr)
    {
